...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* ThemeItems, int count, float width_excess)
{
    if (count == 1)
    {
        if (ThemeItems[0].Width >= 0.0f)
            ThemeItems[0].Width = ImMax(ThemeItems[0].Width - width_excess, 1.0f);
        return;
    }
    ImQsort(ThemeItems, (size_t)count, sizeof(ImGuiShrinkWidthItem), ShrinkWidthItemComparer);
    int count_same_width = 1;
    while (width_excess > 0.0f && count_same_width < count)
    {
        while (count_same_width < count && ThemeItems[0].Width <= ThemeItems[count_same_width].Width)
            count_same_width++;
        float max_width_to_remove_per_item = (count_same_width < count && ThemeItems[count_same_width].Width >= 0.0f) ? (ThemeItems[0].Width - ThemeItems[count_same_width].Width) : (ThemeItems[0].Width - 1.0f);
        if (max_width_to_remove_per_item <= 0.0f)
            break;
        float width_to_remove_per_item = ImMin(width_excess / count_same_width, max_width_to_remove_per_item);
        for (int item_n = 0; item_n < count_same_width; item_n++)
            ThemeItems[item_n].Width -= width_to_remove_per_item;
    ...
    {
        float width_rounded = ImFloor(ThemeItems[n].Width);
        width_excess += ThemeItems[n].Width - width_rounded;
        ThemeItems[n].Width = width_rounded;
    }
    while (width_excess > 0.0f)
        for (int n = 0; n < count && width_excess > 0.0f; n++)
        {
            float width_to_add = ImMin(ThemeItems[n].InitialWidth - ThemeItems[n].Width, 1.0f);
            ThemeItems[n].Width += width_to_add;
...
static bool Items_ArrayGetter(void* data, int indices, const char** out_text)
{
    const char* const* ThemeItems = (const char* const*)data;
    if (out_text)
        *out_text = ThemeItems[indices];
    return true;
}

// Getter for the old Combo() API: "item1\0item2\0item3\0"
static bool Items_SingleStringGetter(void* data, int indices, const char** out_text)
{
...
bool ImGui::Combo(const char* label, int* current_item, const char* const ThemeItems[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)ThemeItems, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::SliderFloat(const char* label, double* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
...
static ImWchar STB_TEXTEDIT_GETCHAR(const ImGuiInputTextState* obj, int indices)                      { return obj->TextW[indices]; }
...
static int  is_word_boundary_from_right(ImGuiInputTextState* obj, int indices)      { if (obj->Flags & ImGuiInputTextFlags_Password) return 0; return indices > 0 ? (is_separator(obj->TextW[indices - 1]) && !is_separator(obj->TextW[indices]) ) : 1; }
static int  is_word_boundary_from_left(ImGuiInputTextState* obj, int indices)       { if (obj->Flags & ImGuiInputTextFlags_Password) return 0; return indices > 0 ? (!is_separator(obj->TextW[indices - 1]) && is_separator(obj->TextW[indices])) : 1; }
static int  STB_TEXTEDIT_MOVEWORDLEFT_IMPL(ImGuiInputTextState* obj, int indices)   { indices--; while (indices >= 0 && !is_word_boundary_from_right(obj, indices)) indices--; return indices < 0 ? 0 : indices; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_MAC(ImGuiInputTextState* obj, int indices)   { indices++; int len = obj->CurLenW; while (indices < len && !is_word_boundary_from_left(obj, indices)) indices++; return indices > len ? len : indices; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_WIN(ImGuiInputTextState* obj, int indices)   { indices++; int len = obj->CurLenW; while (indices < len && !is_word_boundary_from_right(obj, indices)) indices++; return indices > len ? len : indices; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(ImGuiInputTextState* obj, int indices)  { ImGuiContext& g = *obj->Ctx; if (g.IO.ConfigMacOSXBehaviors) return STB_TEXTEDIT_MOVEWORDRIGHT_MAC(obj, indices); else return STB_TEXTEDIT_MOVEWORDRIGHT_WIN(obj, indices); }
...
bool ImGui::ListBox(const char* label, int* current_item, const char* const ThemeItems[], int items_count, int height_items)
{
    const bool value_changed = ListBox(label, current_item, Items_ArrayGetter, (void*)ThemeItems, items_count, height_items);
    return value_changed;
}
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int indices), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
static float Plot_ArrayGetter(void* data, int indices)
{
    ImGuiPlotArrayGetterData* plot_data = (ImGuiPlotArrayGetterData*)data;
    const float v = *(const float*)(const void*)((const unsigned char*)plot_data->Values + (size_t)indices * plot_data->Stride);
    return v;
}
...
void ImGui::PlotLines(const char* label, float (*values_getter)(void* data, int indices), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
...
void ImGui::PlotHistogram(const char* label, float (*values_getter)(void* data, int indices), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
