...
struct ImVec2
{
    ...
    float  operator[] (size_t indices) const    { IM_ASSERT(indices == 0 || indices == 1); return (&x)[indices]; }  // We very rarely use this [] operator, the assert overhead is fine.
    float& operator[] (size_t indices)          { IM_ASSERT(indices == 0 || indices == 1); return (&x)[indices]; }  // We very rarely use this [] operator, the assert overhead is fine.
...
namespace ImGui
{
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol indices, ImU32 col);                        // modify a style color. always use this if you modify the style after NewFrame().
    IMGUI_API void          PushStyleColor(ImGuiCol indices, const ImVec4& col);
    IMGUI_API void          PopStyleColor(int count = 1);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar indices, float val);                     // modify a style float variable. always use this if you modify the style after NewFrame().
    IMGUI_API void          PushStyleVar(ImGuiStyleVar indices, const ImVec2& val);             // modify a style ImVec2 variable. always use this if you modify the style after NewFrame().
    ...
    IMGUI_API ImU32         GetColorU32(ImGuiCol indices, float alpha_mul = 1.0f);              // retrieve given style color with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImU32         GetColorU32(const ImVec4& col);                                 // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImU32         GetColorU32(ImU32 col);                                         // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API const ImVec4& GetStyleColorVec4(ImGuiCol indices);                                // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const ThemeItems[], int items_count, int popup_max_height_in_items = -1);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int indices, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1);
    ...
    IMGUI_API bool          SliderFloat(const char* label, double* v, float v_min, float v_max, const char* format = "%.5f", ImGuiSliderFlags flags = 0);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
    ...
    IMGUI_API bool          ListBox(const char* label, int* current_item, const char* const ThemeItems[], int items_count, int height_in_items = -1);
    IMGUI_API bool          ListBox(const char* label, int* current_item, bool (*items_getter)(void* data, int indices, const char** out_text), void* data, int items_count, int height_in_items = -1);

    // Widgets: Data Plotting
    // - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
    IMGUI_API void          PlotLines(const char* label, const float* values, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
    IMGUI_API void          PlotLines(const char* label, float(*values_getter)(void* data, int indices), void* data, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0));
    IMGUI_API void          PlotHistogram(const char* label, const float* values, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
    IMGUI_API void          PlotHistogram(const char* label, float(*values_getter)(void* data, int indices), void* data, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0));
    ...
    IMGUI_API const char*   GetStyleColorName(ImGuiCol indices);                                    // get a string corresponding to the enum value (for display, saving, etc.).
...
enum ImGuiWindowFlags_{
    ...
    ImGuiWindowFlags_NoResize               = 1 << 1,   // 禁用用户使用右下手柄调整大小
    ImGuiWindowFlags_NoMove                 = 1 << 2,   // 禁止用户移动窗口
    ImGuiWindowFlags_NoScrollbar            = 1 << 3,   // 禁用滚动条（窗口仍然可以用鼠标或以编程方式滚动）
    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 4,   // 禁止用户使用鼠标滚轮垂直滚动。 在子窗口上，鼠标滚轮将转发给父窗口，除非也设置了 NoScrollbar。
    ImGuiWindowFlags_NoCollapse             = 1 << 5,   // 禁用用户通过双击折叠窗口。 也称为窗口菜单按钮（例如在停靠节点内）。
    ImGuiWindowFlags_AlwaysAutoResize       = 1 << 6,   // 每帧调整每个窗口的大小到它的内容
    ImGuiWindowFlags_NoBackground           = 1 << 7,   // 禁用绘图背景颜色（WindowBg 等）和外边框。 类似于使用 SetNextWindowBgAlpha(0.0f)。
    ImGuiWindowFlags_NoSavedSettings        = 1 << 8,   // 从不在 .ini 文件中加载/保存设置
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // 禁用捕捉鼠标，通过传递进行悬停测试。
    ImGuiWindowFlags_MenuBar                = 1 << 10,  // 有一个菜单栏
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // 允许水平滚动条出现（默认关闭）。 您可以使用 SetNextWindowContentSize(ImVec2(width,0.0f)); 在调用 Begin() 之前指定宽度。 阅读“水平滚动”部分的 imgui_demo 中的代码。
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // 从隐藏状态转换为可见状态时禁止获取焦点
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 13,  // 获得焦点时禁用将窗口置于最前面（例如单击它或以编程方式赋予它焦点）
    ...
    ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 16,  // 确保无边框子窗口使用style.WindowPadding（无边框子窗口默认忽略，因为更方便）
    ImGuiWindowFlags_NoNavInputs            = 1 << 18,  // 窗口内没有游戏手柄/键盘导航
    ImGuiWindowFlags_NoNavFocus             = 1 << 19,  // 使用游戏手柄/键盘导航时不关注此窗口（例如，通过 CTRL+TAB 跳过）
    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // 在标题旁边显示一个点。 在选项卡/停靠上下文中使用时，不假定单击 X + 闭包时选择选项卡（将等待用户停止提交选项卡）。 否则当按下 X 时假定关闭，因此如果您继续提交选项卡可能会重新出现在选项卡栏的末尾。
    ImGuiWindowFlags_NoDocking              = 1 << 21,  // 禁用此窗口的停靠
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoSavedSettings            = 1 << 4,   // 在 .ini 文件中禁用持久列顺序、宽度和排序设置。
...
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver  = 1 << 2,   //如果对象/窗口没有永久保存的数据（.ini 文件中没有条目），则设置变量
...
struct ImGuiIO
{
    ...
    ImGuiConfigFlags   ConfigFlags;             // = 0              // 请参阅 ImGuiConfigFlags_ 枚举。 由用户/应用程序设置。 游戏手柄/键盘导航选项等。
    ImGuiBackendFlags  BackendFlags;            // = 0            // 请参阅 ImGuiBackendFlags_ 枚举。 由后端（imgui_impl_xxx 文件或自定义后端）设置以传达后端支持的功能。
    ImVec2      DisplaySize;                    // <unset>          // 主显示尺寸，以像素为单位（通常 == GetMainViewport()->Size）。 可能改变每一帧。
    float       DeltaTime;                      // = 1.0f/60.0f     // 自上一帧以来经过的时间，以秒为单位。 可能改变每一帧。
    float       IniSavingRate;                  // = 5.0f           // 将位置/大小保存到 .ini 文件之间的最短时间，以秒为单位。
    const char* IniFilename;                    // = "imgui.ini"    // const char* 初始化文件名； // = "imgui.ini" // .ini 文件的路径（重要：默认的“imgui.ini”是相对于当前工作目录的！）。 设置 NULL 以禁用自动 .ini 加载/保存，或者如果您想手动调用 LoadIniSettingsXXX() / SaveIniSettingsXXX() 函数。
    ...
    void*       UserData;                       // = NULL         // 存储自己的数据。

    ImFontAtlas*Fonts;                          // <auto>          // 字体图集：加载、栅格化并将一种或多种字体打包到单个纹理中。
    float       FontGlobalScale;                // = 1.0f           // Global scale all fonts
    bool        FontAllowUserScaling;           // = false          // 允许用户使用 CTRL+Wheel 缩放单个窗口的文本。
    ImFont*     FontDefault;                    // = NULL         // 在 NewFrame() 上使用的字体。 使用 NULL 来使用 Fonts->Fonts[0]。
    ...
    // 视口选项（设置 ImGuiConfigFlags_ViewportsEnable 时）
    bool        ConfigViewportsNoAutoMerge;     // = false;         // 设置为使所有浮动的 imgui 窗口始终创建自己的视口。 否则，它们会在重叠时合并到主宿主视口中。 也可以在单个视口上设置 ImGuiViewportFlags_NoAutoMerge。
    bool        ConfigViewportsNoTaskBarIcon;   // = false          // 禁用辅助视口的默认操作系统任务栏图标标志。 当视口不需要任务栏图标时，将在其上设置 ImGuiViewportFlags_NoTaskBarIcon。
    bool        ConfigViewportsNoDecoration;    // = true           // 禁用辅助视口的默认操作系统窗口装饰标志。 当视口不需要窗口装饰时，将在其上设置 ImGuiViewportFlags_NoDecoration。 启用装饰可能会在操作系统级别产生后续问题（例如最小窗口大小）。
    bool        ConfigViewportsNoDefaultParent; // = false          // 为辅助视口禁用主视口的默认操作系统父子关系。 默认情况下，视口标有 ParentViewportId = <main_viewport>，期望平台后端在操作系统窗口之间设置父/子关系（某些后端可能会忽略此）。 如果您希望默认值为 0，则设置为 true，则所有视口都将是顶级操作系统窗口。
    ...
    bool        WantSaveIniSettings;                // 当手动 .ini 加载/保存处于活动状态时 (io.IniFilename == NULL)，这将被设置为通知您的应用程序您可以调用 SaveIniSettingsToMemory() 并自行保存。 重要提示：保存后自行清除 io.WantSaveIniSettings！
...
struct ImDrawList
{
    ...
    inline    void  PrimWriteIdx(ImDrawIdx indices)                                     { *_IdxWritePtr = indices; _IdxWritePtr++; }
